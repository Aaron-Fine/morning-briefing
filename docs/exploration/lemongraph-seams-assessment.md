...
## Appendix A: LLM-vs-deterministic audit
...
> **Line-number freshness.** Re-validated against HEAD on 2026-05-30: all
> 15 findings still exist, but line numbers have drifted a few lines since
> this was first written (e.g. `_normalize_tag` is now `parse.py:180`,
> the `tag_label` override `parse.py:603`, `_ensure_primary_glance_coverage`
> `parse.py:318`, `_rebalance_categories` `analyze_domain.py:757`). Treat
> the cited line numbers as indicative, not exact. The reliable identifier
> is the function/symbol name — grep for it at PR time rather than trusting
> the line. This is the master override-removal checklist referenced by the
> Graph Epic's first success criterion.
